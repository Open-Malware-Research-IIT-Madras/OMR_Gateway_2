...
def CopyToGW1(filehash):
    file_name =str(filehash)
    # Call the shell script with the file name as an argument using subprocess
    try:
        subprocess.run(["./shell.sh", file_name], check=True)
...
def network(source_folder, destination_folder):
    # Path to your shell script
    script_path = "./copynetwork.sh"
    
    try:
        # Call the shell script with arguments using subprocess
       # result = subprocess.run([script_path, source_folder, destination_folder], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        result = subprocess.run([script_path, source_folder, destination_folder], check=True)

        # Print the output from the shell script
        print(result.stdout.decode())
    except subprocess.CalledProcessError as e:#a
        print(f"Error occurred: {e.stderr.decode()}")


def trigger_ssh_script(username, password, ip_address, remote_dir, python_argument):
    # Command to call the shell script with arguments
    subprocess.run([
        'bash', 'triggered.sh',
        username, password, ip_address, remote_dir, python_argument
    ])
...
#    cursor.execute("update omr_data set job_status='processing' where job_id=%s",(jobid,))
   ...
   #here we are moving the file to gateway one 
    CopyToGW1(filehash)
   #now from here we will trigger starttestModified.py with filehash as an paramenter 
    trigger_ssh_script('jugaad', 'jug@@d*%$', '192.168.3.1', '/home/jugaad/jugaad/Malware/GW1/startTest', filehash)
    #network(filehash,filehash)
