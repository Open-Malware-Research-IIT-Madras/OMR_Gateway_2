...
print("Copying the file from the Gateway 1 to the Profiler")

#copy_call_with_args=#
#os.system(
print(sys.argv[1], " These are the arguments (filehash)------------------------------- These are the arguments")

copy_call_with_args="scp /home/jugaad/jugaad/Malware/Profiler_OS_NW/malware_submission/'%s' jugaad@192.168.2.27:/C:/Profiler_OS_NW/malware_submission/" % (str(sys.argv[1]))
print("file transfer from gateway1 to profiler start")
os.system(copy_call_with_args)
print("file transfer from  gateway1 to profiler end")

print("Copy to Profiler successful")
...
def trigger_scp_copy(source_file_path, remote_user, remote_host, remote_directory): 
    print("The scp copy command has been triggered from the trigger_scp_copy function")
...
def runs(row):
    ...
    file=file[0]
    netlog_cmd1_start_GW1_all = "echo \"jug@@d*%$\" |  /home/jugaad/jugaad/Malware/GW1/GW_StartNetworkCapture_all.sh "+file
    print("First command run") 
   #netlog_cmd1_start_GW1_I2 = "echo \"jug@@d*%$\" |  /home/jugaad/jugaad/Malware/GW1/GW1_StartNetworkCapture_I2.sh "+file
    ...
    p1=subprocess.Popen(netlog_cmd1_start_GW1_all, shell=True)
    ...
    #trigger_scp_copy("/home/jugaad/jugaad/Malware/Profiler_OS_NW/omrmalwares/2f6637e5d06ed0c598989455e6448f6d.dat","jugaad","jugaad@192.168.2.27","\Profiler_OS_NW\omrmalwares\2f6637e5d06ed0c598989455e6448f6d.dat")
    
   # p = subprocess.Popen(["/usr/bin/pwsh",  "/home/jugaad/jugaad/Malware/GW1/startTest/protest.ps1", file ],stdout=sys.stdout)
    
   # p = subprocess.Popen(["/usr/bin/pwsh",  "/home/jugaad/jugaad/Malware/GW1/startTest/omrFiles/test.ps1", file],stdin=DEVNULL,stdout=sys.stdout)
    p=subprocess.Popen(["/home/jugaad/jugaad/Malware/GW1/startTest/omrFiles/test.sh",file], stdin=DEVNULL, stdout=sys.stdout)
    p.communicate()
    ...
    p3=subprocess.Popen(netlog_cmd5_stop_GW1, shell=True)
    p3.wait()
    p1.kill()
    time.sleep(5)
